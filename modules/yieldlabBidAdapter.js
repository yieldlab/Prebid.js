...
export const spec = {
  ...
  interpretResponse: function (serverResponse, originalBidRequest) {
    ...
    originalBidRequest.validBidRequests.forEach(function (bidRequest) {
      ...
      if (matchedBid) {
        const adSize = parseSize(matchedBid.adsize)
        ...
        const bidResponse = {
          ...
          referrer: '',
          ad: `<script src="${ENDPOINT}/d/${matchedBid.id}/${bidRequest.params.supplyId}/?ts=${timestamp}${extId}"></script>`
        }
        ...
        if (isVideo(bidRequest)) {
          ...
          bidResponse.vastUrl = `${ENDPOINT}/d/${matchedBid.id}/${bidRequest.params.supplyId}/?ts=${timestamp}${extId}`
